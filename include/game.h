...
private:
    ...
    vector<vector<int>> component_graph; // Graph of components
    int start;
    int player_cnt = 0;
    vector<int> cnt_components_; // Amount of vertices in each component
    vector<int> cycles; // List of components which are cycles <=> have more than one vertex <=> non-terminal outcomes
};
